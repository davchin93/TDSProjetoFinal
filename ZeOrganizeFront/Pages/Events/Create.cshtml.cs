...
namespace ZeOrganizeFront.Pages.Events
{
    public class Create : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
                ...
                eventDate > eventDate.AddDays(7))
            {
                if (!DatesAreValid)
                {
                    ModelState.AddModelError("DatesAreValid", "A data de encerramento deve ser posterior � data e hor�rio de in�cio.");
                }

                if ((eventDate + eventStartsAt) < DateTime.Now)
                {
                    ModelState.AddModelError("DatesAreValid", "A data de in�cio deve ser posterior � data e hor�rio atual.");
                }

                if (eventDate > eventDate.AddDays(7))
                {
                    ModelState.AddModelError("DatesAreValid", "A dura��o do evento n�o pode ultrapassar uma semana.");
                }
